                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="btn-popup pull-right">
                            <button type="button" class="btn btn-primary" data-toggle="modal" data-original-title="test" data-target="#exampleModal">Add Tag</button>
                            <div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                <div class="modal-dialog" role="document">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h5 class="modal-title f-w-600" id="exampleModalLabel">Add Tag</h5>
                                            <button class="close" type="button" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">×</span></button>
                                        </div>
                                        <div class="modal-body">
                                            <form class="needs-validation" method="post" action="{{ route('postTag') }}">
                                                @csrf
                                                <div class="form">
                                                    <div class="form-group">
                                                        <label for="validationCustom01" class="mb-1">Tag Name :</label>
                                                        <input class="form-control" id="validationCustom01" type="text" name="tag_name" required="true">
                                                        <label for="validationCustom01" class="mb-1">Sub Category:</label>
                                                        <select class="form-control" name="subcategory_id" required="">
                                                            @foreach($subcategory as $result)
                                                                <option value="{{ $result->subcategory_id }}">{{ $result->subcategory_name }}</option>
                                                            @endforeach
                                                        </select>
                                                    </div>                                                  
                                                </div>                           
                                ...
                                <div class="jsgrid-grid-header jsgrid-header-scrollbar">
                                    <table class="jsgrid-table">
                                        <tr class="jsgrid-header-row">
                                            <th class="jsgrid-header-cell jsgrid-align-center jsgrid-header-sortable" style="width: 50px;">Tag Id</th>                                            
                                            <th class="jsgrid-header-cell jsgrid-header-sortable" style="width: 50px;text-align: center">Tag Name</th>
                                                ...
                                                @foreach($tag as $result)
                                                    @if($a==0)
                                                        ...
                                                        <tr class="jsgrid-row">
                                                            ...
                                                            <td class="jsgrid-cell" style="width: 50px;">{{ $result->tag_name }}</td>
...
$(document).ready(function(){    
    ...
    $(".jsgrid-delete-button").click(function(){
        ...
        let token = $('input[name=_token]').val();
        var answer = prompt("Please make sure to delete the tag, It will cause your product not working. Type 'DELETE' to confirm (Case sensitive)");
        if(answer == "DELETE"){
            $.post("{{ route('deleteTag') }}",
            {
                id : id,
                _token : token
            },function(data){
